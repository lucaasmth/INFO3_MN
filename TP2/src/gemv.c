...
void
mncblas_sgemv(const MNCBLAS_LAYOUT layout, const MNCBLAS_TRANSPOSE TransA, const int M, const int N, const float alpha,
...
void mncblas_dgemv(MNCBLAS_LAYOUT layout, MNCBLAS_TRANSPOSE TransA, const int M, const int N, const double alpha,
...
#pragma omp parallel for
            for (i=0; i < M; i += incY) {
                sommeligne.real = 0;
                sommeligne.imaginary = 0;
                for (j = 0; j < N; j += incX) {
                    sommeligne = add_complexe_float(sommeligne,
                                                    mult_complexe_float(*(((complexe_float_t *) A) + i * N + j),
                                                                        *(((complexe_float_t *) X) + j)));
                }
                *(((complexe_float_t *) Y) + i) = add_complexe_float(
                        mult_complexe_float(*((complexe_float_t *) alpha), sommeligne),
                        mult_complexe_float(*((complexe_float_t *) beta), *(((complexe_float_t *) Y) + i)));
            }
        } else if (TransA == MNCblasTrans) {
            register complexe_float_t sommeligne;
            register unsigned int i, j;
#pragma omp parallel for
            for (i=0; i < N; i += incY) {
                ...
                for (j = 0; j < M; j += incX) {
                    sommeligne = add_complexe_float(sommeligne,
                                                    mult_complexe_float(*(((complexe_float_t *) A) + j * N + i),
                                                                        *(((complexe_float_t *) X) + j)));
                }
                *(((complexe_float_t *) Y) + i) = add_complexe_float(
                        mult_complexe_float(*((complexe_float_t *) alpha), sommeligne),
...
#pragma omp parallel for
            for (i=0; i < M; i += incY) {
                sommecolonne.real = 0;
                sommecolonne.imaginary = 0;
                for (j = 0; j < N; j += incX) {
                    sommecolonne = add_complexe_float(sommecolonne,
                                                      mult_complexe_float(*(((complexe_float_t *) A) + j * M + i),
                                                                          *(((complexe_float_t *) X) + j)));
                }
                *(((complexe_float_t *) Y) + i) = add_complexe_float(
                        mult_complexe_float(*((complexe_float_t *) alpha), sommecolonne),
                        mult_complexe_float(*((complexe_float_t *) beta), *(((complexe_float_t *) Y) + i)));
            }
        } else if (TransA == MNCblasTrans) {
            register complexe_float_t sommecolonne;
...
#pragma omp parallel for
            for (i=0; i < N; i += incY) {
                ...
                for (j = 0; j < M; j += incX) {
                    sommecolonne = add_complexe_float(sommecolonne,
                                                      mult_complexe_float(*(((complexe_float_t *) A) + i * M + j),
                                                                          *(((complexe_float_t *) X) + j)));
                }
                *(((complexe_float_t *) Y) + i) = add_complexe_float(
                        mult_complexe_float(*((complexe_float_t *) alpha), sommecolonne),
...
#pragma omp parallel for
            for (i=0; i < M; i += incY) {
                ...
                sommeligne.imaginary = 0;
                for (j = 0; j < N; j += incX) {
                    sommeligne = add_complexe_double(sommeligne,
                                                     mult_complexe_double(*(((complexe_double_t *) A) + i * N + j),
                                                                          *(((complexe_double_t *) X) + j)));
                }
                *(((complexe_double_t *) Y) + i) = add_complexe_double(
                        mult_complexe_double(*((complexe_double_t *) alpha), sommeligne),
                        mult_complexe_double(*((complexe_double_t *) beta), *(((complexe_double_t *) Y) + i)));
            }
        } else if (TransA == MNCblasTrans) {
            register complexe_double_t sommeligne;
            register unsigned int i, j;
#pragma omp parallel for
            for (i=0; i < N; i += incY) {
                ...
                for (j = 0; j < M; j += incX) {
                    sommeligne = add_complexe_double(sommeligne,
                                                     mult_complexe_double(*(((complexe_double_t *) A) + j * N + i),
                                                                          *(((complexe_double_t *) X) + j)));
                }
                *(((complexe_double_t *) Y) + i) = add_complexe_double(
                        mult_complexe_double(*((complexe_double_t *) alpha), sommeligne),
...
#pragma omp parallel for
            for (i=0; i < M; i += incY) {
                ...
                sommecolonne.imaginary = 0;
                for (j = 0; j < N; j += incX) {
                    sommecolonne = add_complexe_double(sommecolonne,
                                                       mult_complexe_double(*(((complexe_double_t *) A) + j * M + i),
                                                                            *(((complexe_double_t *) X) + j)));
                }
                *(((complexe_double_t *) Y) + i) = add_complexe_double(
                        mult_complexe_double(*((complexe_double_t *) alpha), sommecolonne),
                        mult_complexe_double(*((complexe_double_t *) beta), *(((complexe_double_t *) Y) + i)));
            }
        } else if (TransA == MNCblasTrans) {
            register complexe_double_t sommecolonne;
...
#pragma omp parallel for
            for (i=0; i < N; i += incY) {
                ...
                for (j = 0; j < M; j += incX) {
                    sommecolonne = add_complexe_double(sommecolonne,
                                                       mult_complexe_double(*(((complexe_double_t *) A) + i * M + j),
                                                                            *(((complexe_double_t *) X) + j)));
                }
                *(((complexe_double_t *) Y) + i) = add_complexe_double(
                        mult_complexe_double(*((complexe_double_t *) alpha), sommecolonne),
